...
def main_worker(args):
    ...
    labeled_xy_avg = []
    lossall=0
    # img_label = load_annotations('./weights/pass50_clu/pixel_attention_new/cluster/train_labeled_52.58.txt')
    img_label = load_annotations('./weights/pass50_newclu2/pixel_clas_clu/test_labeled_54.56.txt')
    ...
    for images, path, height, width in tqdm(dataloader):
        ...
        with jt.no_grad():
            ...
            # labeled3.append("{0} {1} {2} {3} {4}".format(path_0, indices, values2[0], values2[1],loss))
            # print('path_0', path)
    ...
    with open(os.path.join(args.dump_path, "test_54.56all.txt"), "w") as f:
        f.write("\n".join(labeled))
    # with open(os.path.join(args.dump_path, "train_e33.txt"), "w") as f:
    #     f.write("\n".join(labeled2))
    # with open(os.path.join(args.dump_path, "new2ddtrain_labeled_e25_2.txt"), "w") as f:
    #     f.write("\n".join(labeled3))
    # with open(os.path.join(args.dump_path, "val_labeled_xy_avg_all.txt"), "w") as f:
    #     f.write("\n".join(labeled_xy_avg_all))
    # with open(os.path.join(args.dump_path, "val_labeled_xy_avg.txt"), "w") as f:
    #     f.write("\n".join(labeled_xy_avg))
