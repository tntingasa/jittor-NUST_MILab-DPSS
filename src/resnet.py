...
class ResNet(nn.Module):
    def __init__(self,
        ...
        if train_mode == 'pixelattn':
            ...
            #                             num_classes + 1, 1, 1)
            # self.last_layer = UPerHead(in_channels=[64, 128, 256, 512],
            #                            in_index=[0, 1, 2, 3],
            #                            pool_scales=(1, 2, 3, 6),
            #                            channels=512,
            #                            dropout_ratio=0.1,
            ...
            self.last_layer = FCNHead(in_channels=512,
                                      in_index=3,
                                      channels=128,
                                      num_convs=2,
                                      concat_input=True,
    ...
    def execute(self, inputs, gridq=None, gridk=None, mode='train'):
        ...
        if self.train_mode == 'finetune':
            # print('进入resnet--finetune!!!')  ###
            out = self.execute_backbone(inputs)
            # print('out', (out).shape)
            
            # print('self.last_layer(out)',self.last_layer(out).shape)
