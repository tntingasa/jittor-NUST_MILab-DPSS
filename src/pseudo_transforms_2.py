...
class Compose(object):
    ...
    def __call__(self, *data):
        assert len(data) == 3
        img, semantic,semantic_2 = data

        for t in self.transforms:
            if 'RandomResizedCropSemantic' in t.__class__.__name__:
                img, semantic,semantic_2 = t(img, semantic,semantic_2)
                ...
                img = t(img)
        return img, semantic,semantic_2
...
class RandomHorizontalFlipSemantic(object):
    ...
    def __call__(self, img:Image.Image, semantic:Image.Image,semantic_2:Image.Image):
        if not isinstance(img, Image.Image):
            img = to_pil_image(img)
        if not isinstance(semantic, Image.Image):
            semantic = to_pil_image(semantic)
        if not isinstance(semantic_2, Image.Image):
            semantic_2 = to_pil_image(semantic_2)
        if random.random() < self.p:
            return img.transpose(Image.FLIP_LEFT_RIGHT), semantic.transpose(Image.FLIP_LEFT_RIGHT),semantic_2.transpose(Image.FLIP_LEFT_RIGHT)
        return img, semantic,semantic_2
...
class RandomResizedCropSemantic(object):
    ...
    def __call__(self, img, semantic,semantic_2):
        """
        Args:
        ...
        return F_pil.resized_crop(img, i, j, h, w, self.size, self.interpolation), \
               ...
               F_pil.resized_crop(semantic_2, i, j, h, w, self.size, Image.NEAREST)
...
class ToTensorSemantic:
    def __call__(self, pic, semantic,semantic_2):
        """
        Args:
        ...
        semantic_2 = jt.array(np.asarray(semantic_2).astype(np.float32)).transpose(2, 0, 1)
        return to_tensor(pic), semantic,semantic_2
