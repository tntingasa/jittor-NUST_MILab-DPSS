...
class Processor:
    def __init__(self, params, num_cls=51):
        ...
        tag = '_'.join(
            [os.path.basename(self.pseudo_path), os.path.basename(self.sam_path), os.path.basename(self.method)])
        self.save_path = self.pseudo_path.replace('_crf', '')
    ...
    def generate_merged_masks(self):
        def process_file(file_path, save_path, sam_path, merger):
            name = file_path.split('.png')[0].split('/')[-1]
            # img_folder = sam_path.split('/')[-1]
            # if os.path.isfile(save_path + f"/{name}.png"):
            #     return
            # print(filename.path)
            img = Image.open(file_path)
            predict_t = np.array(img)[:, :, 0]
            # if img_folder in ['n02104029', 'n02119022', 'n02281406', 'n02747177',
            #                   'n06794110', 'n03201208', 'n04404412', 'n03891251']:
            #     w, h = img.size
            #     if w > 900 or h > 900:
            #         img = img.resize((512, 512))
            #         predict_t = np.array(img)[:, :, 0]
            # else:predict_t = np.array(img)[:, :, 0]
            predict = predict_t
        ...
        if self.multithread:
            with ThreadPoolExecutor() as (executor):
                futures = []
                # print('re:',self.save_path)
                save_path = self.save_path
                sam_path = self.sam_path
                list1 = os.listdir(self.pseudo_path)
                ...
                for file in list1:
                    ...
                    self.save_path = os.path.join(self.save_path, file)
                    self.sam_path = os.path.join(self.sam_path, file)
                    Path(self.save_path).mkdir(parents=True, exist_ok=True)
                    # print(file)
                    # print('save_path:',self.save_path)
                    # print('sam_path',self.sam_path)
                    list2 = os.listdir(os.path.join(self.pseudo_path, file))
                    list2.sort(key=lambda x: int(x[15:-4]))
                    for filename in list2:
                    # for filename in os.scandir(file):
                        # print(filename)
                        file_path = os.path.join(self.pseudo_path, file,filename)
                        if os.path.isfile(file_path):
                            future = executor.submit(process_file, file_path, self.save_path, self.sam_path, self.merger)
            ...
            # Path(self.save_path).mkdir(parents=True, exist_ok=True)
            for file in os.scandir(self.pseudo_path):
                self.save_path = os.path.join(self.save_path, file)
                self.sam_path = os.path.join(self.sam_path, file)
                Path(self.save_path).mkdir(parents=True, exist_ok=True)
                for filename in os.scandir(file):
    ...
    def generate_samples(self):
        ...
        if selected_files is None or len(selected_files) == 0:
            ...
            for f in selected_files:
                ...
                image_rows.append(image_row)
                tag = '_'.join([os.path.basename(self.pseudo_path), os.path.basename(self.sam_path),
                                os.path.basename(self.method)])
                if self.vis_worse:
                    tag = tag + '/worst'
                elif self.vis_best:
                    tag = tag + '/best'
