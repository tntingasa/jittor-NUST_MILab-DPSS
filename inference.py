...
def main_worker(args):
    ...
    # build dataset
    assert args.mode in ['validation', 'test', 'train']
    ...
    for images, path, height, width in tqdm(dataloader):
        ...
        with jt.no_grad():
            ...
            output = (output + jt.flip(output_h, dim=(3,))) / 2
            # else:
            # images_v = jt.flip(images, dim=(2, ))
            # output_v = model(images_v)
            # output = (output + jt.flip(output_v, dim=(2, ))) / 2

            ###384###
            images_384 = nn.resize(images, (384, 384), mode='bilinear', align_corners=False)
            output3 = model(images_384)
            images_384_h = jt.flip(images_384, dim=(3,))
            output_384_h = model(images_384_h)
            output3 = (output3 + jt.flip(output_384_h, dim=(3,))) / 2

            ###512###
            images_512 = nn.resize(images, (512, 512), mode='bilinear',align_corners=False)
            output2 = model(images_512)
            images_512_h = jt.flip(images_512, dim=(3,))
            output_512_h = model(images_512_h)
            output2 = (output2 + jt.flip(output_512_h, dim=(3,))) / 2
            ####
            if H * W > args.max_res * args.max_res and args.max_res > 0:
                output = nn.interpolate(output, (args.max_res, int(args.max_res * W / H)), mode="bilinear",
                                        align_corners=False)
                output2 = nn.interpolate(output2, (args.max_res, int(args.max_res * W / H)), mode="bilinear",
                                        align_corners=False)

                # output3 = nn.interpolate(output3, (args.max_res, int(args.max_res * W / H)), mode="bilinear",
                #                         align_corners=False)

                output = jt.argmax((output+output2)/2, dim=1, keepdims=True)[0]
                ...
                output2 = nn.interpolate(output2, (H, W), mode="bilinear", align_corners=False)
                # output3 = nn.interpolate(output3, (H, W), mode="bilinear", align_corners=False)
                prediction = jt.argmax((output+output2)/2, dim=1, keepdims=True)[0]
