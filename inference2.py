...
def main_worker(args):
    ...
    dump_path = os.path.join(args.dump_path, 'result2')
    # dump_path = args.dump_path

    targets = []
    predictions = []
    txt1_path = './label2.txt'
    txt1_path_fen = './label2_fen.txt'
    for images, path, height, width in tqdm(dataloader):
        ...
        with jt.no_grad():
            ...
            if H * W > args.max_res * args.max_res and args.max_res > 0:
                ...
                out1, out2 = jt.argmax((output + output2) / 2, dim=1, keepdims=True)
            ...
            if max(result) <= 0:
                ...
                fen = float(result[label])
            with open(txt1_path, 'a') as f:
                f.write(os.path.join(cate, name + '.JPEG') + ' ' + str(label) + '\n')
            with open(txt1_path_fen, 'a') as f:
                f.write(os.path.join(cate, name + '.JPEG') + ' ' + str(label)+ ' ' + str(fen) + '\n')
            # if H * W > args.max_res * args.max_res and args.max_res > 0:
            #     output = nn.interpolate(output, (args.max_res, int(args.max_res * W / H)), mode="bilinear",
            #                             align_corners=False)
