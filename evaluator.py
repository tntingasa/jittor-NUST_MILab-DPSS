...
def get_dataset(args, mode, threshold=None, match=None):
    gt_path = os.path.join(args.data_path, f'validation-segmentation')
...
def evaluator(args, mode, num_classes, thresholds, matches):
    ...
    for t in thresholds:
        ...
        for target, boundary_target, predict, boundary_predict, _ in tqdm(loader):
            ...
            IoUDifferentSizeGPUWithBoundary(predict.reshape(-1), target.reshape(-1), boundary_predict.reshape(-1),
                                            boundary_target.reshape(-1), num_classes + 1, Ts, Ps, TPs, BTs, BPs, BTPs)
        ...
        print(
            ...
            'S: {:.2f}\tMS: {:.2f}\tML: {:.2f}\tL: {:.2f}\tBS: {:.2f}\tBMS: {:.2f}\tBML: {:.2f}\tBL: {:.2f}'.
                format(t, ACC, mIoU, mBIoU, FMeasure, mIoUs[0], mIoUs[1], mIoUs[2], mIoUs[3], mBIoUs[0], mBIoUs[1],
                       mBIoUs[2], mBIoUs[3])
        )
