    ...
    for image_folder in list1:
        ...
        for filename in list2:
            ...
            if mask.shape != norm.shape:
                mask = mask.reshape(np.shape(norm))
            if cv2.countNonZero(mask) < 1000:
                # 利用cv2.minMaxLoc寻找到图像中最亮和最暗的点
                gray = cv2.GaussianBlur(norm, (args.radius, args.radius), 0)
                (minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray)
            else:
                msk_cov = cv2.add(norm.astype(np.uint8), np.zeros(np.shape(norm), dtype=np.uint8), mask=mask)
                # 利用cv2.minMaxLoc寻找到图像中最亮和最暗的点
                gray = cv2.GaussianBlur(msk_cov, (args.radius, args.radius), 0)
                (minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray)

            cv2.circle(img, maxLoc, 5, (255, 0, 0), 2)
            cv2.imwrite(save_base1, img)
