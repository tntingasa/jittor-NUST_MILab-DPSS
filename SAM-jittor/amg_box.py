...
def main(args: argparse.Namespace) -> None:
    ...
    for image_folder in list1:
        ...
        if not os.path.exists(os.path.join(args.output2, image_folder)):
            os.makedirs(os.path.join(args.output2, image_folder))
        for filename in list2:
            base = os.path.basename(filename)
            base = os.path.splitext(base)[0]
            save_base = os.path.join(args.output, image_folder, base)
            save_base2 = os.path.join(args.output2, image_folder, filename)
